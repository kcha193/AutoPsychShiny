...
kable(facet.df, format="latex", booktabs=T, align = 'l', caption = "Summary of Coding for Facet Variable", longtable=T) %>%
  kable_styling(latex_options = c("striped","repeat_header"), font_size = 11)
...
facet.df.results  %>%                                                           
  dplyr::mutate(
    ...
    `facet.z` = cell_spec(`facet.z`, "latex", color = ifelse(`facet.z` >= facet.z.cut | `facet.z` <= -facet.z.cut, "red", "black")),
    `facet.p` = cell_spec(`facet.p`, "latex", color = ifelse(`facet.p` <= facets.cut.p, "red", "black")),
    `Overall Group Effect` = cell_spec(`Overall Group Effect`, "latex", color = ifelse(`Overall Group Effect` >= facets.cut.logit | `Overall Group Effect` <= -facets.cut.logit, "blue", "black"))
               ) %>%
  dplyr::select(`parameter`, facet, `xsi`, `se.xsi`, facet.z, `facet.p`, `Overall Group Effect`) %>%
  kable(format="latex", escape = F, booktabs = T, linesep = "", align='cc', caption= "Examination of DIF through Facets Analysis", longtable=T) %>%
  kable_styling(latex_options = c("striped", "repeat_header"), font_size = 10) %>%
  footnote(general = "(a) Statistically significant results in red, practically significant results in blue.")
