...
title:  | 
  ...
  |
  | 
  | Technical Report, version 1PLM_FACETS_v1.0
  |
  |
  | 
  |
...
full.resp <- read.csv("facet.p.resp.csv")         # Note: First row of df is a facet.
...
NA.Delete <- T
p.fit.threshold <- 3.0
node.sequence <- seq(-6, 6, len= 21)
conv <- .0001
maxiter <- 1000
color.choice <- "Eurasian Steppe"
binwidth <- 0.25

# Facets Analysis
Facets.cut.p <- 0.05
Facets.cut.logit <- 0.5
...
# if (params$rendered_by_shiny){
#     shiny::setProgress(0.96)  # Kevin is cool
# }
```
...
## A8. Facet Analysis 
This section provides a summary of the results of the facets analysis. The facet of interest was the "`r colnames(facet)`" variable. This analysis was undertaken with the intention to identify possible item bias due "`r colnames(facet)`" effects. The frequency with which each "`r colnames(facet)`" facet occured for each case is presented in Table 11.  
...
```{r CHUNK49_Facets_analysis, echo=FALSE, fig.height=7, fig.width=10, message=FALSE, warning=FALSE, include=FALSE}
...
# Facets.cut.p is global parameter
# Facets.cut.logit is global parameter
```


Many facets analysis was undertaken to investigate the effect of "`r colnames(facet)`" differential item function (DIF) on all items in accordance with Equation 9. Results of the full analysis are provided in Table 12. Results are broken into two sections in the table. The first `r facet.number` rows reflect the overall effect of the facet categories on item difficulty in `r Construct`. The rows beyond the first `r facet.number` rows represent the effect of the student grouping facet on each of the `r ncol(resp)` test item difficulties.  
