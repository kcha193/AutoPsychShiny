...
start.time <- Sys.time()
mod1 <- readxl::read.xlsx(params$datapath_A, sheet = "Item Results")
mod2 <- readxl::read.xlsx(params$datapath_B, sheet = "Item Results")
resp2 <- readxl::read.xlsx(params$datapath_B, sheet = "Item Response Data")
Person1 <- readxl::read.xlsx(params$datapath_A, sheet = "Ordered Person Results")
...
ggplot2::ggplot(EQ_df, aes(x = `Form A Deltas`, y = `Form B Deltas`)) + 
            ...
            geom_smooth(method = lm, 
                        ...
                        fill = "#69b3a2", 
                        se = TRUE) +
            geom_label_repel(aes(label = `Common Link Items`),
                             box.padding   = 0.35, 
                             point.padding = 0.5,
...
EQ_df  %>%                                                           
  dplyr::mutate(
    ...
    `Standardized Diff. (z)` = cell_spec(`Standardized Diff. (z)`, 
                                     "latex", 
  ...
  kable(format="latex", escape = F, booktabs = T, linesep = "", align='cc', caption= "Formal Check for Item Difficulty Invariance", longtable=T) %>%
  kable_styling(latex_options = c("striped", "repeat_header"), font_size = 11) %>%
  footnote(general = "Statistically significant results (with z-scores equal to or above |1.96|) are highlighted in red.") 
...
mu <- plyr::ddply(df, "Test_Form", summarise, grp.mean=mean(Equated_Theta))
...
thr.df <- tam.threshold(mod2.mod1.equated)

Plausible.values <- tam.pv(mod2.mod1.equated)
