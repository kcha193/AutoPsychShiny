...
title:  | 
  ...
  |
  | 
  | Technical Report, version 1PLM_1DIM_FACETS 0.1.0
  |
  |
  | 
  |
...
start.time <- Sys.time()
full.resp <- read.csv(params$datapath)

resp <- full.resp[, -1]                           # Remove facet column to run regular 1dim 1plm report (facet model to run in the appendix)
facet <- as.data.frame(full.resp[, 1])            # Subset facet df to enable facet report in the additional appendix
colnames(facet) <- colnames(full.resp)[1]         # Rename first column in facet df so as to prepare it for analysis in appendix

facet.df <- as.data.frame(table(facet))           # Generate tabular results to present in appendix
colnames(facet.df)[1] <- colnames(full.resp)[1]   # Rename the facet df first column the facet of interest
...
facets.cut.p <- params$facets.cut.p
facets.cut.logit <- params$facets.cut.logit
...
#### 7. Adjust placeholders (blank strings) for actual relevant string objects
if(Construct == ""){
...
#### 8. Give rendering progress in UI to communicate with user throughout report ####
...
#### 1. Load packages #### (done via trial tabs.R for this app)
if (params$rendered_by_shiny){
...
r <- CTT::reliability(resp, NA.Delete = NA.Delete)  # User chooses option
...
#### 2. Change to English ####
poor.disc.n.Eng <-  as.character(english::as.english(poor.disc.n))
...
mod1 <- TAM::tam.mml(resp, control=list(nodes=node.sequence, conv = conv, maxiter = maxiter), constraint = constraint)
end.mod.t <- Sys.time()
...
ggWrightMapnew <<- function(theta, b, binwidth = 0.3, color = "blue", size = 2, item.names){
  ...
  g1 <- ggplot(df.theta, aes_string(x = "theta")) + 
    ...
    ShinyItemAnalysis::theme_app(base_size = size) +
...
item.fit.mod1 <- TAM::msq.itemfit(mod1) 
...
Ability.theta <- TAM::tam.wle(mod1)$theta
...
ctt_raw <- TAM::tam.ctt(resp, EAP.person)
...
```{r CHUNK21a_Facets_analysis, echo=FALSE, fig.height=7, fig.width=10, message=FALSE, warning=FALSE, include=FALSE}
# Establish facet formula #* note TAM::tam.mml Example 8, base formula
facet.name <- colnames(facet)
string.formula <- paste("~item*", facet.name, sep="")
formulaA <- as.formula(string.formula)

# Run facet model
facet.results <- TAM::tam.mml.mfr(resp = resp,
                                  facets = facet, 
                                  formulaA = formulaA, 
                                  constraint = constraint, 
                                  control=list(nodes=node.sequence, conv = conv, maxiter = maxiter)
                                  )
# Extract facet results
facet.df.results <- facet.results$xsi.facets
facet.df.results <- facet.df.results[-c(1:ncol(resp)), ]
rownames(facet.df.results) <- NULL

# Create df
facet.z <- facet.df.results$xsi / facet.df.results$se.xsi
facet.df.results <- cbind.data.frame(facet.df.results, facet.z)
options(scipen=999)
facet.p <- NCmisc::Z.to.p(facet.z)
facet.df.results <- cbind.data.frame(facet.df.results, facet.p)
facet.df.results <- cbind.data.frame(facet.df.results, facet.df.results$xsi*2)
colnames(facet.df.results)[7] <- "Overall Group Effect"
facet.df.results[,3:7] <- apply(facet.df.results[,3:7], 2, FUN=function(x)round(x, 3))

facet.number <- as.character(english::as.english(length(table(facet))))
Facet.caption.name <- paste("Effect of the ", facet.name, " Facet on Item Difficulty", sep = "")

# Establish flag cut points
facet.z.cut <- NCmisc:::p.to.Z(facets.cut.p)

# Extract group effect
facet.df.results.a <- facet.df.results[-c(1,2), ]                            # remove first 2 rows
facet.df.results.b <- facet.df.results.a[1:(nrow(facet.df.results.a)/2), ]   # extract first 10 rows

stat.sig.facet.n <- as.character(english::as.english(sum(facet.df.results.b$facet.p <= facets.cut.p)))
prac.sig.facet.n <- as.character(english::as.english(sum(facet.df.results.b$`Overall Group Effect` >= facets.cut.logit)))

# Grammar
if(stat.sig.facet.n == "one"){
  item.statsig.si.pl <- "item"
} else if (!stat.sig.facet.n == "one"){
  item.statsig.si.pl <- "items"
}

if(prac.sig.facet.n == "one"){
  item.pracsig.si.pl <- "item"
} else if (!prac.sig.facet.n == "one"){
  item.pracsig.si.pl <- "items"
}

if (params$rendered_by_shiny){
    shiny::setProgress(0.41)
}
```
...
Many facets Rasch analysis was also carried out on the data to check for item bias. The facet of interest was `r colnames(facet)`. Analysis checked for items that exhibited an overall group effects of `r facets.cut.logit` logits and associated p values of less than `r facets.cut.p`. To this end, of the `r ncol(resp)` items, `r prac.sig.facet.n` `r item.pracsig.si.pl` exhibited an overall group effect of `r facets.cut.logit` logits, while `r stat.sig.facet.n` `r item.statsig.si.pl` exhibited p values less than the pre-specified cut of value of `r facets.cut.p`. Details can be found in Appendix 8, Table 12.  
...
```{r CHUNK21b_Item_table, echo=FALSE, fig.height=7, fig.width=10, message=TRUE, warning=FALSE}
knitr::kable(Item.table, format = "latex", booktabs = T, align = 'l', caption = "List of Items", longtable = T) %>%
  kableExtra::kable_styling(latex_options = c("striped", "scale_down", "repeat_header"), font_size = 11)
...
knitr::kable(Crony, "latex", booktabs = T, align = 'l', caption = "Interpreting the Cronbach's Alpha") %>% 
...
#### 3.4.1 The one-parameter logistic (Rasch) model   

\hfill\break
The one parameter logistic model (1PLM), or Rasch model, is one of a family of Item Response Theory (IRT) models that support the design, analysis, and scoring of tests, questionnaires, and other instruments that measure abilities or attributes (Wright & Mok, 2004). When item-response data is dichotomous (i.e., right or wrong; 0 or 1), and it is assumed that one dimension or trait (such as numeracy ability) is being measured, a unidimensional Rasch model can be written as,  
  
\begin{equation}
P(X_{ni}=1|\vartheta_n,\delta_i)=\frac{e^{[D(\vartheta_n-\delta_i)]}}{1+e^{[D(\vartheta_n-\delta_i)]}}
\end{equation}  
  
\vspace{4mm}
where $P(X_{ni}=1|\vartheta_n,\delta_i)$ is student $n$'s probability of scoring 1, rather than 0, on item $i$, given person $n$'s fixed ability and item $i$'s fixed difficulty; $e$ is the mathematical constant, approximately 2.718; $D$ is the scaling factor of 1.702 in order to match the logistic and probit metrics very closely (Lord & Novick, 1968) (for simplicity, this constant can be removed from subsequent formulae). 

In the Rasch model, the probability of student $n$ obtaining a correct response to item $i$ is a function of student ability ($\vartheta_n$) and item difficulty ($\delta_i$). Under the family of Rasch models, student ability estimates ($\vartheta$) are special estimates of students' percentage correct scores while item difficulty estimates are special transformations of item percentage incorrect scores. The transformation of such scores involves converting percentage values to log-odds estimates enabling the formation of a single uniform logit scale for which both students and items can be placed.  
  
Mathematically, when $\vartheta_n$ matches $\delta_i$ (i.e., $\vartheta-\delta=0$), the probability of student scoring 1 is 50:50, or p = .50; consider, $\frac{e^{\vartheta-\delta}}{1+e^\vartheta-\delta}=.50$. However, when $\vartheta-\delta=1$, for example, p = .73, and when $\vartheta-\delta=-1$, p = .27. For the dichotomous Rasch model, the numerator, $1+e^{\vartheta=\delta}$ represents the probability of score 0 and 1.  
  
We can make our notation more explicit to define person $n$'s probability of scoring 1 on item $i$ as follows (Masters & Wright, 1984), 

\begin{equation}
P(X_{ni}=1|\vartheta_n,\delta_i)=\frac{e^{(\vartheta_n-\delta_{i1})}}{1+e^{(\vartheta_n-\delta_{i1})}}
\end{equation}  
  
\vspace{4mm}
##### 3.4.1.1 Extending the Rasch model to acocunt for polytomous responses  
The dichotomous unidimensional Rasch model has been extended to account for polytomous item responses such as 0, 1 and 2 (Andrich, 1978; Masters, 1982). In a general sense, the dichotomous Rasch model is extended to account for polytomous responses by representing each pair of adjacent scores as a string of ordered categories,  
  
\begin{equation}
P(X_{nij}=x|\vartheta_n,\delta_{ij})=\frac{e^{(\vartheta_n-\delta_{ij})}}{1+e^{(\vartheta_n-\delta_{ij})}}, j = 1, 2, ..., m_{i}.
\end{equation}  
  
\vspace{4mm} 
where $j$ specifies the step to be taken by person $n$ in item $i$ from the lower of the adjacent score category to the higher category. Instead of defining a single item-response function for an item, the partial credit model defines $m_i$ category-response functions for each item--each function representing the conditional probability of student $n$ completing $j$ steps in item $i$ given that they complete either $j-1$ of $j$ (Masters and Wright, 1984).  

##### 3.4.1.2 Master's partial credit Rasch model 
Currently, the most popular extension of the Rasch model to account for polytomous items responses is the partial credit Rasch model (PCM, Masters, 1982). The model was designed to account for instances when partial credit (i.e., 0, 1, 2) scores can be awarded for increasing levels of student competence. 
  
In Masters' (1982) PCM model, the probability of each individual score category is derived for difference levels of student ability. At each level of theta, the sum of the respective score categories is zero.

In the case of a partial credit scoring item with 0, 1, and 2 as possible scores, the probability of scoring $1$ as opposed to $0$ or $1$ is given by,  
  
\begin{equation}
P(1/0,1)=\frac{e^{(\vartheta_n-\delta_{i1})}}{1+e^{(\vartheta_n-\delta_{i1})}}
\end{equation} 

\vspace{4mm}  

and the probability of scoring $2$ as opposed to $1$ or $2$ is given by,
  
\begin{equation}
P(2/1,2)=\frac{e^{(\vartheta_n-\delta_{i2})}}{1+e^{(\vartheta_n-\delta_{i2})}}
\end{equation} 

\vspace{4mm}

While the derivation of Masters' (1982) PCM is based on adjacent score probabilities, the probability for each score for different levels of student ability ($\vartheta$) can also be derived. For the example, for partial credit item with 0, 1, and 2 scores, the probability of person $n$ achieving score $1$ is given by,  
  
\begin{equation}
P(X=1)=\frac{e^{(\vartheta_n-\delta_{i1})}}{[1+e^{(\vartheta_n-\delta_{i1})}]+[e^{(2\vartheta_n-(\delta_{i1}+\delta_{i2}))}]}
\end{equation} 
  
\vspace{4mm}
and the probability of person $n$ achieving score $2$ is given by,  
  
\begin{equation}
P(X=1)=\frac{e^{(2\vartheta_n-(\delta_{i1}+\delta_{i2}))}}{[1+e^{(\vartheta_n-\delta_{i1})}]+[e^{(2\vartheta_n-(\delta_{i1}+\delta_{i2}))}]}
\end{equation} 

where the $m-1$ Partial Credit Model (PCM) delta ($\delta$) parameters reflect the point at which the probability of being in category $k$ and category $k-1$ are equal.  
  
As an example, substituting $\vartheta$ = -1, 0, and 1 into Equation X, we find that the probability of scoring Category 1 is $p=.468$, $p=.576$ and $p=.468$, respectively. However, it should be noted that the PCM delta parameters may not be ordered when the probability of a middle response category (e.g., 1 in 0, 1, 2 scores) are not common (see Wu, Tam, Jen, 2016, pp. 164-166). Thurstonian gammas ($\gamma$) (thresholds) provide a more appropriate alternative where $\gamma$ is defined as the ability at which the probability of achieving score $k$ or higher reaches $p=0.50$.  
  
Therefore, Thurstonian $\gamma$'s can be viewed as developmental "cut points" for dividing ability into a continuum of score regions (Wu, Tam, Jen, 2016). Also, it should be noted that in the case of the dichotomous Rasch model, Thurstonian $\gamma$'s are equivalent to $\delta$ parameters. 
  
Other polytomous Rasch models have been devised however, Master's PCM model has become prominent due to it's flexibility. Andrich (1978a; 1978b) proposed an alternative to the Partial Credit Model (Masters & Wright, 1984) where response categories are assigned intersection parameters (thresholds) that are considered equal across items. However, this is generally considered quite restrictive and only appropriate for polytomous data with fixed anchors across all items, such as "disagree", "neutral", and "agree" response categories. 
  
##### 3.4.1.3 Parameter Estimation
Because the dichotomous Rasch model is just a special case of the PCM, modern software packages, such as the open-source TAM package (Robitzsch, Kiefer, & Wu, 2018), can model both dichotonous and polytomous item scores (or mixtures of such scores). After the data is collected, item $\delta$ and person $\vartheta$ parameters can be estimated. This is commonly done in two steps with item $\delta$ parameters estimated independently of $\vartheta$ (model calibration step), thereafter, while fixing item $\delta$s person ability $\vartheta$s are then estimated in a latter step.
  
##### 3.4.1.4 Model Calibration  
Model calibration involves maximizing the full model likelihood ($L(\vartheta,\delta)$) with regards to all item $\delta$ parameters. The model callibration method chosen herein is marginal maximum likelihood (MML) which assumes a prior distribution (commonly a normal distribution) for student ability estimates. Marginal maximum likelihood (MML) estimation (see Expectation Maximization algorithm, Bock & Aitkin, 1981) was used as this estimation method has been shown to produce more stable person and item estimates related to the associated population. The assumed discretized population profile (prior distribution) of the sample students can be pre-set. In the TAM package, this is a standard normal distribution with 21 nodes ranging from $theta=-6$ to $theta=6$. Unlike other Rasch model estimation methods (i.e., JML and CML), the MML method can handle perfect item and person response patterns (i.e., attribute an estimate of $\vartheta$ or $delta$) making this callibration method quite useful in various data situations. 

##### 3.4.1.5 Student ability estimation
Once the item $\delta$ parameters are estimated via MML, they are set as fixed values making student ability estimation possible. There are several specific methods for estimating student ability. Here we make use of the $expected a priori$ or EAP student ability estimation ($\hat{\vartheta}_{EAP}$) (Bock & Mislevy, 1982). In the aforementioned $TAM$ package ($version\hspace{.1cm}2.10{\text -}24$), the EAP point ability estimates are based on the mean of the posterior distribution. The EAP ability point estimates have been proven to be less biased than other estimates of $theta$ (e.g., those based on JML estimation, see Wu, Tam, & Jen, 2016, pp. 267-271 for further details). 
  
IRT analysis for this report was undertaken with the assistance of the $TAM$ package ($version\hspace{.1cm}2.10{\text -}24$; Robitzsch, Kiefer, & Wu, 2018). The $TAM$ package enables automated analysis of either dichotomous or polytomous (partial credit; i.e., 0, 1, 2) scored data.  
  
##### 3.4.1.6 Summary of Rasch Approach  
The 1PL (or Rasch) modeling approach used here was chosen due to its flexibility. Application of Masters' (1982) PCM provides an application to a wide variety of data conditions encompassing dichotomous, polytomous, and mixed item-response scores. Masters (1982) PCM model is also suitable for agreement scales, essay rating criterion, and questions and skills where students can demonstrate increasing levels of competence. Use of the MML calibration also provides flexibility due perfect item or person response options, and EAP person ability estimates offer overall less biased estimates for person ability.  
  
This specific flexible approach to modeling student performance (and other person attributes) within a Rasch framework has many advantages. Unlike other modeling appraoches (such as the 2PL, 3PL, and 4PL, see Magis, Yan, & von Davier, 2017) the Rasch approach outlined here provides "a coherent and verifiable support for the traditional measurement practice of forming raw scores by counting events. What these models add is control over this traditional practice" (Masters & Wright, p. 542).  
    
#### 3.4.2 IRT estimates of reliability  
\hfill\break
According to Robitzsch, Kiefer, and Wu (2018), if we let $v$ denote the variance of theta ($\vartheta$) estimates and let $s$ denote the average of the squared error for those estimates, then, the EAP reliability is defined as $v/(v+s)$, while the WLE reliability is defined as $(v-s)/v$. Therefore, the EAP formula sums total variance in the denominator.  
...
where, $x_{ni}$ is the observed score for each $n$ person for each item $i$, $E(X_{ni})$ represents the expected or theoretical expectation for the respective observed score, and the $Var(X_{ni})$ is given by $P_{ni}(1-P_{ni})$ which represents the variance asscoiated with each item ($i$) person ($n$) response. Unlike the item outfit statistic ($fit_{unwt}$), the item infit statistic $fit_{wt}$ identifies an average degree of variance for each item (see denominator, $\sum_n Var(X_{ni})$).  
...
#### 3.4.11 Many-Facets Rasch Analysis (DIF)
\hfill\break 
Many facets Rasch analysis can be undertaken to explore the effects of student group membership on item difficulty. This form of analysis can be used to identify items that might be bias toward certain student groups. Items that appear especially bias may be flagged for removal from the test or instrument. The many-facets Rasch model employed in this report extends Equation 4 to include a student group ($g$) by item ($i$) interaction term and is presented in Equation 9.  
 
\begin{equation}
P(x_{ni}=1|\vartheta_n,\delta_i)=\frac{e^{(\vartheta_n-(\delta_i + G_{g} + D_{gi}))}}{1+e^{(\vartheta_n-(\delta_i + G_{g} + D_{gi}))}}
\end{equation}  

Where partial credit scoring applied, the same $D_{gi}$ element can be included to the right of the $\delta_i$ element in the partial credit model (Equation 5). 
  
Basically, Equation 9 specifies that the probability of success on each item depends on an overall adjustment to the difficulty of the item as a consequence of the membership of students in a group ($G_{g}$). In addition to the overall adjustment to item difficulty as a conseuqnece of group membership, another adjustment is made, respectively, for each item, $D_{gi}$. If, for example, an item favours male students, the $D_{gi}$ element (reflecting membership to the male student group) will be negative, reflecting the fact that the item is easier for males. Conversely, a negative value for the male group membership would also reflect item bias against female counterparts. It should be noted that DIF is not associated with overall performance differences between groups as the sum of $D_{gi}$ across all elements is zero. Further, if the effect of male group membership on an item is $D_{gi}=-.2$, then, conversely, the effect of female group membership on the same items would be $D_{gi}=.2$ making the overall difference between groups, 0.4 logits. 
  
In addition to the reporting of $D_{gi}$ for each item, with negative values suggesitng that the item favours the focal group, standard errors for each $D_{gi}$ are also given. By dividing the $D_{gi}$ estimate by its standard error, we are able to generate a z-statistic. The z-statistic can be transformed to a p value and hence a level of statistical significance can be determined for each DIF estimate. Though, statistical significance is often reached when sample sizes are very large so practical significance is also important. An overall difference in logits of 0.5 is commonly used as a cut-off value (Wu, Tam, & Jen, p. 215). There are different appraoches to dealing with items that exhibit DIF. Items exhibiting DIF may be removed, split as items for different groups, and left in the test (see Wu, Tam, & Jen, pp. 217-223 for a complete discussion).  
  
For this analysis, the cut point for identifying practical significance between groups is set at `r facets.cut.logit` logits, while the level of statistical significance is set at p = `r facets.cut.p`. 
...
The $CTT$ package provided results pertaining to the frequency of response categories, point biserial (Pearson) discrimination indices, poly- and bi-serial discrimination indices, the Cronbach's alpha ($\alpha$) reliability coefficient, and the classical test theory statistics including the $xtable$ and $Hmisc$ packages provided results pertaining to the full Pearson correlation matrix (between all items). The $TAM$ package was used to carry out the 1PLM model for the partial credit data. Finally, the $ShinyItemAnalysis$ package provided a visual illustration for the relative positions of question items and person abilities.  
...
knitr::kable(freq.uneven, format = "latex", booktabs = T, align = 'cc', caption = "Frequency of Categories for Each Item", longtable = T) %>%
...
disc.disc1 %>%                                                           
  dplyr::mutate(                                                      # mutate function enables the piping of extra coloring info into the table
    ...
    `Point Biserial or Pearson` = kableExtra::cell_spec(`Point Biserial or Pearson`, "latex", color = ifelse(`Point Biserial or Pearson` < 0.000, "red", 
                                                                                                 ifelse(`Point Biserial or Pearson` < disc.threshold, "blue", "black"))),
    `Lower CI` = kableExtra::cell_spec(`Lower CI`, "latex", color = ifelse(`Lower CI` < 0.000, "red",
                                                                                 ifelse(`Lower CI` < disc.threshold, "blue", "black"))),
    `Upper CI` = kableExtra::cell_spec(`Upper CI`, "latex", color = ifelse(`Upper CI` < 0.000, "red",
                                                                                 ifelse(`Upper CI` < disc.threshold, "blue", "black"))),
    `Bi- or Poly-serial` = kableExtra::cell_spec(`Bi- or Poly-serial`, "latex", color = ifelse(`Bi- or Poly-serial` < 0.000, "red",
  ...
  knitr::kable(format="latex", escape = F, booktabs = T, linesep = "", align='cc', caption= "Point-Biserial/Pearson, and Bi-/Poly-Serial Discrimination Indices", longtable=T) %>%
  kableExtra::kable_styling(latex_options = c("striped", "repeat_header"), font_size = 11) %>%
  kableExtra::footnote(general = "Correlations under predefined limit (if any) in blue while negative values (if any) in red.")
...
round.item.fit.stats  %>%                                                           
  dplyr::mutate(
    ...
    `Outfit t` = kableExtra::cell_spec(`Outfit t`, "latex", color = ifelse(`Outfit t` >= 1.96 | `Outfit t` < -1.96, "red", "black")),
    `Outfit p` = round.item.fit.stats$`Outfit p`,
    `Infit` = round.item.fit.stats$Infit,
    `Infit t` = kableExtra::cell_spec(`Infit t`, "latex", color = ifelse(`Infit t` >= 1.96 | `Infit t` < -1.96, "red", "black")),
    `Infit p` = round.item.fit.stats$`Infit p`
  ) %>%
  dplyr::select(`Item Abbreviation`, Outfit, `Outfit t`, `Outfit p`, Infit, `Infit t`, `Infit p`) %>%
  knitr::kable(format="latex", escape = F, booktabs = T, linesep = "", align='cc', caption= "Item-fit Statistics", longtable=T) %>%
  kableExtra::kable_styling(latex_options = c("striped", "repeat_header"), font_size = 11) %>%
  kableExtra::footnote(general = "Statistically significant results (equal to or above t = |1.96|) are highlighted in red.") 
...
The item difficulties (deltas) had a mean of `r mean.item.diff` and a standard deviation of `r sd.item.diff`. The difficulty logits for each respective item are provided in Table 6. If the item is dichotomous, the item's respective difficulty estimate is the level of ability at which there is an equal probability of the student to obtain a score of 1. If the item is polytomous, e.g., scored 0, 1, 2, then the item's respective difficulty estimate is the level of ability at which the probability to obtain half the maximum score for that item (i.e., 1, in this instance). These respective item difficulty estimates can be used when undertaking fixed anchor equating.  
  
```{r CHUNK32_Item_difficulty, echo=FALSE, fig.height=12, fig.width=16, message=TRUE, warning=FALSE}
knitr::kable(item.diff.df, format = "latex", booktabs = T, align = 'cc', caption = "Item Difficulties", longtable = T) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "scale_down", "repeat_header"), font_size = 11)
...
plot(mod1, type = "expected", low = -6, high = 6)
...
knitr::kable(thr, format = "latex", booktabs = T, align = 'cc', caption = "Thurstonian Thresholds", longtable = T) %>%
  kableExtra::kable_styling(latex_options = c("striped", "repeat_header", font_size = 11)) 
...
## 5. REFERENCES  
Adams, R. J., Wu, M. L., Wilson, M. (2012) The Rasch rating model and the disordered threshold controversy. Educational Psychological Measurement, 72, 547–573.   
...
Note that in the Table 8, N represents the number of person responses for that item, M represents the mean raw score across all persons for that item, xsi represent the item deltas for that item, AXsi values represent the point at which adjacent score categories have equal probability. Note that it is normal for some disorderedness when the probability of reaching a middle category is small (where few persons reached that category). As Wu, Tam, and Jen explain, "this phenomenon of dis-ordering of the [delta] parameters is one disadvantage of using the delta (d) parameters to interpret item responses in relation to ability" (p. 165). For a full explanation of this phenomenon, see Wu, Tam, and Jen (2016, pp. 162 to 165, and Adams, Wu, & Wilson, 2012). Wu, Tam, and Jen (2016, p. 170) summarise that, for partial credit items, Thurstonian thresholds can be more useful indicators of score difficulties (see subsections 3.4.6 and 4.9 for details on Thurstonian thresholds).  
...
knitr::kable(Item.cat.param, format = "latex", booktabs = T, align = 'cc', caption = "Item Delta Parameters", longtable = T) %>%
  kableExtra::kable_styling(latex_options = c("striped", "scale_down", "repeat_header"), font_size = 11)
...
ctt_raw.df %>%                                                           
  dplyr::mutate(                                        # mutate function enables the piping of extra coloring info into the table
    ...
    Pbs = kableExtra::cell_spec(Pbs, "latex", color = ifelse(`Disorderedness T/F` == TRUE, "red", "black")),
    ...
    knitr::kable(format = "latex", escape = F, booktabs = T, linesep = "", align = 'cc', caption = "CTT Item Analysis", longtable = T) %>%
    kableExtra::kable_styling(latex_options = c("striped", "repeat_header"), font_size = 9) %>%
    kableExtra::footnote(general = "Disorderedness point bi-serials, where lower category correlation is larger (toward 1.00), in red.")
...
The test information and standard error curve graph is presented in Figure 6. Test information at the lower end of theta was estimated at `r TI.lower` whilst test information at the higher end was estimated at `r TI.upper`. Test information was maximised where $\vartheta$=`r max.theta` (test information = `r TI.max.r`). The standard errors for theta (which have an inverse relationship to the test information) are also presented in Figure 5. The standard error at the lower end of theta was estimated at `r SE.lower` and at the higher end of theta was `r SE.upper`, and, where test information was maximised, the minimum standard error was `r SE.min.r`.  
...
knitr::kable(missingness.df, format = "latex", booktabs = T, align = 'cc', caption = "Missingness for Each Item", longtable = T) %>%
  kableExtra::kable_styling(latex_options = c("striped","repeat_header"), font_size = 11) #%>%
...
##### COMPILE #####
list.of.data <- list("Ordered Person Results" = Person.stats.o, "Ordered Thurstonian thresholds" = Thresh.vec, 
                     "Person Results" = Person.stats, "Item Results" = Item.details, 
                     "Individual Item Results" = item.diff.df, "Item Thresholds" = thr.df, 
...
openxlsx::write.xlsx(list.of.data, file = params$excel_name)
...
## A8. Facet Analysis 
This section provides a summary of the results of the facets analysis. The facet of interest was the "`r colnames(facet)`" variable. This analysis was undertaken with the intention to identify possible item bias due to student classification for the "`r colnames(facet)`" variable The frequency with which each "`r colnames(facet)`" facet occured for each student is presented in Table 11.  
  
```{r CHUNK49_facet, echo=FALSE, message=TRUE, warning=FALSE}
knitr::kable(facet.df, format="latex", booktabs=T, align = 'l', caption = "Summary of Coding for Facet Variable", longtable=T) %>%
  kableExtra::kable_styling(latex_options = c("striped","repeat_header"), font_size = 11)

if (params$rendered_by_shiny){
    shiny::setProgress(0.97)
}
```


Many facets analysis was undertaken to investigate the effect of "`r colnames(facet)`" differential item function (DIF) on all items in accordance with Equation 9. Results of the full analysis are provided in Table 12. Results are broken into two sections in the table. The first `r facet.number` rows reflect the overall effect of the facet categories on average item difficulty in the `r Construct` construct. The rows beyond the first `r facet.number` rows represent the effect of the student grouping facet on each of the `r ncol(resp)` test item difficulties.  
  
```{r CHUNK49_Facets_table_test, echo=FALSE, message=TRUE, warning=FALSE}
# parameter  facet  xsi  se.xsi  facet.z   facet.p  Overall Group Effect
facet.df.results  %>%                                                           
  dplyr::mutate(
    `parameter` = facet.df.results$parameter,
    `facet` = facet.df.results$facet,
    `xsi` = facet.df.results$xsi,
    `se.xsi` = facet.df.results$se.xsi,
    `facet.z` = kableExtra::cell_spec(`facet.z`, "latex", color = ifelse(`facet.z` >= facet.z.cut | `facet.z` <= -facet.z.cut, "red", "black")),
    `facet.p` = kableExtra::cell_spec(`facet.p`, "latex", color = ifelse(`facet.p` <= facets.cut.p, "red", "black")),
    `Overall Group Effect` = kableExtra::cell_spec(`Overall Group Effect`, "latex", color = ifelse(`Overall Group Effect` >= facets.cut.logit | `Overall Group Effect` <= -facets.cut.logit, "blue", "black"))
               ) %>%
  dplyr::select(`parameter`, facet, `xsi`, `se.xsi`, facet.z, `facet.p`, `Overall Group Effect`) %>%
  knitr::kable(format="latex", escape = F, booktabs = T, linesep = "", align='cc', caption= "Examination of DIF through Facets Analysis", longtable=T) %>%
  kableExtra::kable_styling(latex_options = c("striped", "repeat_header"), font_size = 10) %>%
  kableExtra::footnote(general = "(a) Statistically significant results in red, practically significant results in blue.")
...
## A9. Summary of Input Parameters, Analysis, and Outputs
Table 13 provides a summary of focal information, settings, and technical outputs for the analysis undertaken for this report.  
 

```{r CHUNK50_Technical_summary, echo=FALSE, message=TRUE, warning=FALSE}
end.time <- Sys.time()
...
Input.measure <- c(Construct, Population, nrow(resp), ncol(resp), NA.Delete, Cronbachs, EAP.reliability, model.deviance, disc.threshold, ci.level, p.fit.threshold, "1PLM_FACETS 0.1.0", constraint, 
                   "1", node.sequence1, maxiter, conv, "autopsych 0.1.0", mod.time_sec, full.rep.time)

summary.df <- cbind.data.frame(Input.setting, Input.measure)
colnames(summary.df) <- c("Report Criteria", "Setting/Ouput")

knitr::kable(summary.df, format="latex", booktabs=T, align = 'l', caption = "Technical Settings for Report", longtable=T) %>%
  kableExtra::kable_styling(latex_options = c("striped","repeat_header"), font_size = 11)
