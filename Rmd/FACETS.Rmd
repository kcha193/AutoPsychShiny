...
\begin{equation}
P(X_{ni}=1|\theta_n,\delta_i)=\frac{e^{[D(\theta_n-\delta_i)]}}{1+e^{[D(\theta_n-\delta_i)]}}
\end{equation}  
  
\vspace{4mm}
where $P(X_{ni}=1|\theta_n,\delta_i)$ is student $n$'s probability of scoring 1, rather than 0, on item $i$, given person $n$'s fixed ability and item $i$'s fixed difficulty; $e$ is the mathematical constant, approximately 2.718; $D$ is the scaling factor of 1.702 in order to match the logistic and probit metrics very closely (Lord & Novick, 1968) (for simplicity, this constant can be removed from subsequent formulae). 

In the Rasch model, the probability of student $n$ obtaining a correct response to item $i$ is a function of student ability ($\theta_n$) and item difficulty ($\delta_i$). Under the family of Rasch models, student ability estimates ($\theta$) are special estimates of students' percentage correct scores while item difficulty estimates are special transformations of item percentage incorrect scores. The transformation of such scores involves converting percentage values to log-odds estimates enabling the formation of a single uniform logit scale for which both students and items can be placed.  
  
Mathematically, when $\theta_n$ matches $\delta_i$ (i.e., $\theta-\delta=0$), the probability of student scoring 1 is 50:50, or p = .50; consider, $\frac{e^{\theta-\delta}}{1+e^\theta-\delta}=.50$. However, when $\theta-\delta=1$, for example, p = .73, and when $\theta-\delta=-1$, p = .27. For the dichotomous Rasch model, the numerator, $1+e^{\theta=\delta}$ represents the probability of score 0 and 1.  
...
\begin{equation}
P(X_{ni}=1|\theta_n,\delta_i)=\frac{e^{(\theta_n-\delta_{i1})}}{1+e^{(\theta_n-\delta_{i1})}}
\end{equation}  
...
\begin{equation}
P(X_{nij}=x|\theta_n,\delta_{ij})=\frac{e^{(\theta_n-\delta_{ij})}}{1+e^{(\theta_n-\delta_{ij})}}, j = 1, 2, ..., m_{i}.
\end{equation}  
...
\begin{equation}
P(1/0,1)=\frac{e^{(\theta_n-\delta_{i1})}}{1+e^{(\theta_n-\delta_{i1})}}
\end{equation} 
...
\begin{equation}
P(2/1,2)=\frac{e^{(\theta_n-\delta_{i2})}}{1+e^{(\theta_n-\delta_{i2})}}
\end{equation} 

\vspace{4mm}

While the derivation of Masters' (1982) PCM is based on adjacent score probabilities, the probability for each score for different levels of student ability ($\theta$) can also be derived. For the example, for partial credit item with 0, 1, and 2 scores, the probability of person $n$ achieving score $1$ is given by,  
  
\begin{equation}
P(X=1)=\frac{e^{(\theta_n-\delta_{i1})}}{[1+e^{(\theta_n-\delta_{i1})}]+[e^{(2\theta_n-(\delta_{i1}+\delta_{i2}))}]}
\end{equation} 
...
\begin{equation}
P(X=1)=\frac{e^{(2\theta_n-(\delta_{i1}+\delta_{i2}))}}{[1+e^{(\theta_n-\delta_{i1})}]+[e^{(2\theta_n-(\delta_{i1}+\delta_{i2}))}]}
\end{equation} 

where the $m-1$ Partial Credit Model (PCM) delta ($\delta$) parameters reflect the point at which the probability of being in category $k$ and category $k-1$ are equal, as illustrated in Figure X.  

[Figure X]

As an example, substituting $\theta$ = -1, 0, and 1 into Equation X, we find that the probability of scoring Category 1 is $p=.468$, $p=.576$ and $p=.468$, respectively. However, it should be noted that the PCM delta parameters may not be ordered when the probability of a middle response category (e.g., 1 in 0, 1, 2 scores) are not common (see Wu, Tam, Jen, 2016, pp. 164-166). Thurstonian gammas ($\gamma$) provide a more appropriate alternative where $\gamma$ is defined as the ability at which the probability of achieving score $k$ or higher reaches $p=0.50$ (See Figure XX).  
...
\begin{equation}
P(x_{ni}=1|\theta_n,\delta_i)=\frac{e^{(\theta_n-(\delta_i + G_{g} + D_{gi}))}}{1+e^{(\theta_n-(\delta_i + G_{g} + D_{gi}))}}
\end{equation}  
