...
title:  | 
  ...
  |
  | 
  | Technical Report, version 1PLM_1DIM 0.1.0
  |
  |
  | 
  |
  ...
  | 
author:  | 
  | Courtney, M. G. R., Chang, K. C-T., Mei, B., Meissel, K., Rowe, L., & Issayeva, L. (2021).
...
start.rows <- c(1, start.rows[-length(start.rows)])
start.rows <- unname(start.rows)                    # first row for each  
...
The full set of item difficulties, that is, the level of student ability where adjacent score categories (e.g., 0-1, 1-2, 2-3) have an equal probability, are provided in the "Item Results" tab of the associated outputted xlsx file. 
...
Note. The Wright Map function is adapted from Martinková and Drabinová's ggWrightMap function from the ShinyItemAnalysis package (2018).  
...
knitr::opts_chunk$set(echo = FALSE)
plot(mod1, type = "items", lwd = 4, low = -6, high = 6)
...
## A8. Summary of Input Parameters, Analysis, and Outputs
